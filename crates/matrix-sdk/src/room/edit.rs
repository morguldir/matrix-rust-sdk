...
use ruma::events::poll::start::PollStartEventContentWithoutRelation;
use ruma::events::poll::unstable_start::{NewUnstablePollStartEventContent, NewUnstablePollStartEventContentWithoutRelation, ReplacementUnstablePollStartEventContent, UnstablePollStartContentBlock, UnstablePollStartEventContent};
    ...
    match new_content {
        ...
        EditedContent::PollStart(text, new_content) => {
            ...
            let replacement = UnstablePollStartEventContent::Replacement(
                ReplacementUnstablePollStartEventContent::plain_text(text, new_content, event_id.to_owned())
            );
            Ok(AnyMessageLikeEventContent::UnstablePollStart(UnstablePollStartEventContent::from(replacement)))
        }
    }
}
